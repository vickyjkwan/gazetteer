view: sf__leads_and_contacts_pdt {
  derived_table: {
    datagroup_trigger: snowflake_sfdc_lead_and_contact_data
    sql:
      WITH leads as (
      SELECT
        'LEAD' as object_type,
      ...
      SELECT
        ...
        c.sub_region_c
      FROM
        SEGMENT.salesforce.contacts c
        LEFT JOIN
        SEGMENT.salesforce.leads as l ON (l.converted_contact_id = c.id)
      WHERE
        NOT c.is_deleted
      )
