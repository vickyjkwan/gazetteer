...
view: sfbase__cases {
  ...
  dimension: github_ticket{
    type: string
    sql: ${TABLE}.git_hub_ticket_c ;;
    description: "Related Github issue, more recently used for Jira"
  }

  dimension: jira_ticket {
    description: "Based on Git_hub_ticket_c in SFDC"
    type: string
    link: {
      label: "View JIRA ticket"
      url: "{{value}}"
      icon_url: "https://docker.atlassian.net/favicon.ico"
    }
    sql: CASE WHEN ${TABLE}.git_hub_ticket_c NOT LIKE '%github%' THEN CONCAT('https://docker.atlassian.net/browse/', ${github_ticket})
    ELSE NULL END;;
  }
  ...
  measure: average_days_open {
    type: average
    sql: DATEDIFF(days, ${TABLE}.created_date, COALESCE(${TABLE}.closed_date, CURRENT_DATE()));;
    drill_fields: [detail*]
  }
